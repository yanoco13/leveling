...
public class TaskService {
    ...
    public List<TaskResponse> findAll() {
        return taskRepository.findAll().stream().map(t -> new TaskResponse(t.getId(), t.getTitle(),
                t.getCategory(), t.getStartDate(), t.getEndDate(), t.isDone(), t.getCreateTime()))
                .toList();
    }
    ...
    public TaskResponse findById(Integer id) {
        ...
        return new TaskResponse(task.getId(), task.getTitle(), task.getCategory(),
                task.getStartDate(), task.getEndDate(), task.isDone(), task.getCreateTime());
    }

    public TaskResponse create(CreateTaskRequest req) {
        Task task =
                new Task(req.title(), req.category(), req.startDate(), req.endDate(), req.isDone());
        Task saved = taskRepository.save(task);
        return new TaskResponse(saved.getId(), saved.getTitle(), saved.getCategory(),
                saved.getStartDate(), saved.getEndDate(), saved.isDone(), saved.getCreateTime());
    }
